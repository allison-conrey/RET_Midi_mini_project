...
class TransformerModel(nn.Transformer):
    ...
    def __init__(self, vocab, emsize, nhead, nhid, nlayers, dropout=0.5):
        super(TransformerModel, self).__init__(
            d_model=emsize, nhead=nhead, dim_feedforward=nhid, num_encoder_layers=nlayers)
        self.vocab = vocab
        self.emsize = emsize
        self.ntoken = len(self.vocab)
        self.model_type = 'Transformer'
        self.src_mask = None
        self.pos_encoder = PositionalEncoding(self.emsize, dropout)

        self.input_emb = nn.Embedding(self.ntoken, self.emsize)
        self.decoder = nn.Linear(self.emsize, self.ntoken)
...
def main(argv):
    ...
    if MODEL != 'RNN':
        model = TransformerModel(vocab=vocab, emsize=16, nhead=4, nhid=1, nlayers=1, dropout=conf.dropout).to(device)
